/*
 * JVM dispatch bridge: CryptoProvider → JCA → PureK fallback.
 *
 * This file is the SINGLE place where dispatch branching lives on JVM.
 * It contains NO @Secret-annotated parameters, so the detekt ConstantTimeRule
 ...
 *
 * The consuming app can inject a provider backed by Android Keystore or other
 * native crypto at runtime (via setCryptoProvider). When no provider is
 * injected (or a provider returns null), the bridge tries the JCA path. If JCA
 * is unavailable, the bridge returns null and the actual falls back to PureK.
 *
 * Android has its own bridge at androidMain/CryptoBridge.kt — KMP source sets
 * do not inherit across targets, so the JCA code is duplicated (identical logic).
 */
 ...
 *
 * JDK 21's JCA does not expose SHAKE256 with a variable-length output API suitable for this
