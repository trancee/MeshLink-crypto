...
/**
 * Android dispatch verification (ADR-0002).
 *
 * Verifies that the public API (JCA dispatch → PureK fallback) produces correct results on Android
 * by checking against RFC known-answer vectors. The commonTest InteropHarnessTest already proves
 * dispatch == PureK; these tests prove the end-to-end path (native or fallback) yields the correct
 * cryptographic result.
 *
 * On the Android unit-test JVM (JDK), JCA provides all algorithms, so the native path always
 * succeeds. On physical Android < API 29, X25519, Ed25519, and ChaCha20-Poly1305 fall back to
 * PureK. The KAT vectors verify correctness for both paths regardless of which one executes.
 */
internal class DispatchVerificationTest {
  ...
  // ------------------------------------------------------------------
  // SHA-256 (RFC 6234 §B.1) — JCA on all APIs
  // ------------------------------------------------------------------
  @Test
  ...
  // ------------------------------------------------------------------
  // SHA-512 (RFC 6234 §B.1) — JCA on all APIs
  // ------------------------------------------------------------------
  @Test
  ...
  // ------------------------------------------------------------------
  // HMAC-SHA-256 (RFC 4231 §4.2.1) — JCA on all APIs
  // ------------------------------------------------------------------
  @Test
  ...
  // ------------------------------------------------------------------
  // HKDF-SHA-256 (RFC 5869 Appendix A.1) — uses native HMAC internally
  // ------------------------------------------------------------------
  @Test
  ...
  // ------------------------------------------------------------------
  // X25519 (RFC 7748 §5.2) — JCA on API 29+, PureK fallback on API < 29
  // ------------------------------------------------------------------
  @Test
  ...
  // ------------------------------------------------------------------
  // Ed25519 sign (RFC 8032 §7.1 Test 1 — empty message) — JCA on API 29+,
  // PureK fallback on API < 29
  // ------------------------------------------------------------------
  @Test
  ...
  // ------------------------------------------------------------------
  // ChaCha20-Poly1305 AEAD (RFC 8439 §2.8) — JCA on API 29+, PureK fallback
  // on API < 29
  // ------------------------------------------------------------------
  @Test
