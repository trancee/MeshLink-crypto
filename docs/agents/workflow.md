...
The `pre-commit` hook runs `spotlessCheck` (ktfmt formatting) on changed files.
It is a surf, not a hard block. A formatting failure prints a warning and exits 0.
