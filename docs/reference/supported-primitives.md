...
| SHA3-256 | [FIPS 202 §6.1](https://nvlpubs.nist.gov/nistpubs/FIPS/NIST.FIPS.202.pdf) | Yes | JCA (`MessageDigest`, JDK 9+) | JCA (`MessageDigest`, API 28+) | Pure-K only |
| SHA3-512 | [FIPS 202 §6.2](https://nvlpubs.nist.gov/nistpubs/FIPS/NIST.FIPS.202.pdf) | Yes | JCA (`MessageDigest`, JDK 9+) | JCA (`MessageDigest`, API 28+) | Pure-K only |
...
On Android, SHA-256, SHA-512, SHA3-256, and SHA3-512 are available on all supported API levels (21+ via JCA, with SHA3 requiring Conscrypt on API 28+). HMAC-SHA256 and HKDF-SHA256 are available on all API levels (21+). X25519, Ed25519, and ChaCha20-Poly1305 require API 29 or higher for the JCA native path. On devices below that level, the library falls back to the pure-Kotlin implementation automatically.
...
| SHA3-256 | API 28 (Conscrypt) | Pure-K |
| SHA3-512 | API 28 (Conscrypt) | Pure-K |
...
iOS targets reach native crypto through Kotlin/Native interop with Apple C frameworks. SHA-256, SHA-512, HMAC-SHA256, and HKDF use CommonCrypto and Security.framework C APIs directly. SHA3-256 and SHA3-512 have no cinterop-accessible C API on iOS (CryptoKit is Swift-only, corecrypto SHA3 is private); the pure-Kotlin Keccak-f[1600] path is always used.
...
| SHA3-256 | 32 bytes | N/A |
| SHA3-512 | 64 bytes | N/A |
